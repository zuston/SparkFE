...
import com._4paradigm.hybridse.spark.{NativeSparkConfig, PlanContext, SparkInstance}
import com._4paradigm.hybridse.spark.utils.{AutoDestructibleIterator, HybridseUtil, SparkColumnUtil, SparkUtil}
...
//      skewDf.repartition(keyScala.map(skewDf(_)): _*)
  ...
  def createComputer(partitionIndex: Int,
    ...
    if (sqlConfig.print) {
      val isSkew = sqlConfig.skewMode == NativeSparkConfig.SKEW
  ...
  def unsafeWindowAggIter(computer: WindowComputer,
    ...
    if (config.skewTagIdx != 0) {
      sqlConfig.skewMode = NativeSparkConfig.SKEW
    ...
    val resIter = if (sqlConfig.skewMode == NativeSparkConfig.SKEW) {
  ...
  def windowAggIterWithUnionFlag(computer: WindowComputer,
    ...
    val resIter = inputIter.flatMap(row => {
      ...
      if (isValidOrder(orderKey)) {
        ...
        if (unionFlag) {
          // primary
          if (sqlConfig.skewMode == NativeSparkConfig.SKEW) {
            val tag = row.getInt(config.skewTagIdx)
            val position = row.getInt(config.skewPositionIdx)
            if (tag == position) {
              Some(computer.compute(row, orderKey, config.keepIndexColumn, config.unionFlagIdx))
