...
import com._4paradigm.hybridse.spark.api.{NativeDataframe, NativeSession}
...
class SQLBaseSuite extends SparkTestSuite {
  ...
  final private val rootDir = {
    ...
    if (pwd.getAbsolutePath.endsWith("native-spark")) {
  ...
  def testCase(sqlCase: SQLCase): Unit = {
    test(SQLBaseSuite.getTestName(sqlCase)) {
      ...
      // TODO: may set config of Map("nativespark.group.partitions" -> 1)
      val spark = new NativeSession(getSparkSession)
      ...
      sqlCase.getInputs.asScala.foreach(desc => {
        ...
        NativeDataframe(spark, df).createOrReplaceTempView(name)
