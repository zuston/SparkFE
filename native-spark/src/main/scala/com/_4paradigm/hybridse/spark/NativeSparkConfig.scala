...
class NativeSparkConfig extends Serializable {

  @ConfigOption(name="nativespark.group.partitions", doc="Default partition number used in group by")
  var groupPartitions: Int = 0

  @ConfigOption(name="nativespark.dbName", doc="Database name")
  ...
  @ConfigOption(name="nativespark.test.tiny", doc="控制读取表的数据条数，默认读全量数据")
  var tinyData: Long = -1

  @ConfigOption(name="nativespark.test.print.sampleInterval", doc="每隔N行打印一次数据信息")
  var printSampleInterval: Long = 100 * 100

  @ConfigOption(name="nativespark.test.print.printContent", doc="打印完整行内容")
  var printRowContent = false

  @ConfigOption(name="nativespark.test.print", doc="执行过程中允许打印数据")
  var print: Boolean = false

  // 数据倾斜优化
  @ConfigOption(name="nativespark.mode", doc="默认normal模式, 如果需要针对数据倾斜优化，需要改成skew")
  var skewMode: String = "normal"

  @ConfigOption(name="nativespark.skew.cnt.name", doc="倾斜条数字段名")
  var skewCntName = "key_cnt_4paradigm"

  @ConfigOption(name="nativespark.skew.tag.name", doc="tag字段名")
  var skewTag = "tag_4paradigm"

  @ConfigOption(name="nativespark.skew.position.name", doc="position字段名")
  var skewPosition = "position_4paradigm"

  @ConfigOption(name="nativespark.skew.watershed", doc="针对key的分水岭，默认是100*100条，才做数据倾斜优化")
  var skewCnt: Int = 100

  @ConfigOption(name="nativespark.skew.ratio", doc="""
      | 数据倾斜因子，如果某个key占总数据量比例一半，那么认为需要优化。范围是0 ~ 1""")
  var skewRatio: Double = 0.5

  @ConfigOption(name="nativespark.skew.level", doc="""
      | 数据倾斜优化级别，默认是1，数据拆分两份分别计算，优化1倍。
      | 因为数据按照2的n次方拆分。所以不建议level改太大""")
  var skewLevel: Int = 1

  // 慢速执行模式
  @ConfigOption(name="nativespark.slowRunCacheDir", doc="""
  ...
  // 窗口数据采样
  @ConfigOption(name="nativespark.window.sampleMinSize", doc="Minimum window size to trigger sample dumping")
  var windowSampleMinSize: Int = -1

  @ConfigOption(name="nativespark.window.sampleOutputPath", doc="Window sample output path")
  var windowSampleOutputPath: String = _

  @ConfigOption(name="nativespark.window.parallelization", doc="Enable window compute parallelization optimization")
  var enableWindowParallelization: Boolean = false

  @ConfigOption(name="nativespark.window.sampleFilter", doc="""
  ...
  @ConfigOption(name="nativespark.window.sampleBeforeCompute", doc="Dump sample before window computation")
  ...
  @ConfigOption(name="nativespark.enable.native.last.join", doc="Enable native last join or not")
  var enableNativeLastJoin = true

  @ConfigOption(name="nativespark.enable.unsaferow.optimization", doc="Enable UnsafeRow optimization or not")
  var enableUnsafeRowOptimization = false

}


object NativeSparkConfig {

  // 数据倾斜优化
  final val SKEW = "skew"

  def fromSparkSession(sess: SparkSession): NativeSparkConfig = {
    ConfigReflections.createConfig(sess.conf)((conf, name) => {
      conf.getOption(name).orElse(conf.getOption("spark." + name)).orNull
    })
  }

  def fromDict(dict: Map[String, Any]): NativeSparkConfig = {
