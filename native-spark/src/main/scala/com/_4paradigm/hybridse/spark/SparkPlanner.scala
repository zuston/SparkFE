...
import com._4paradigm.hybridse.spark.nodes.{ConstProjectPlan, DataProviderPlan, GroupByAggregationPlan, GroupByPlan, JoinPlan, LimitPlan, RenamePlan, RowProjectPlan, SimpleProjectPlan, UnsafeRowProjectPlan, WindowAggPlan}
import com._4paradigm.hybridse.spark.utils.{HybridseUtil, GraphvizUtil, NodeIndexInfo, NodeIndexType}
...
class SparkPlanner(session: SparkSession, config: NativeSparkConfig) {
  ...
  def this(session: SparkSession) = {
    this(session, NativeSparkConfig.fromSparkSession(session))
  }
  ...
  private def withSQLEngine[T](sql: String, db: Database, config: NativeSparkConfig)(body: SQLEngine => T): T = {
    ...
    if (config.enableWindowParallelization) {
      ...
      logger.info("Disable window parallelization optimization, enable by setting nativespark.window.parallelization")
    }
