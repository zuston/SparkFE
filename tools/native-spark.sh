...
DEFAULT_NATIVE_SPARK_JAR=$(find ${CUR_DIR}/../java | grep native-spark-.*-with-dependencies.jar)
NATIVE_SPARK_JAR=${NATIVE_SPARK_JAR:=${DEFAULT_NATIVE_SPARK_JAR}}
JVM_PROFILER_JAR=""
SPARK_JARS=""
USE_SPARK_SUBMIT=0
USE_SPARK_BENCH=0
...
while [[ ${i} -le $# ]]; do
    ...
    case ${key} in
        ...
        --native-spark-jar)     NATIVE_SPARK_JAR=${value}; let i++;;
...
echo "[Configurations]"
echo "- application jar: ${NATIVE_SPARK_JAR}"
echo "- application arguments: ${APP_ARGS}"

if [[ ${NATIVE_SPARK_JAR} == "" ]]; then
...
if [[ ${USE_SPARK_SUBMIT} -eq 0 ]]; then
    ...
    if [[ ${USE_SPARK_BENCH} -eq 1 ]]; then
        benchspark spark-submit ${SPARK_SUBMIT_ARGS} \
             --class Main ${NATIVE_SPARK_JAR} ${APP_ARGS}
    else
