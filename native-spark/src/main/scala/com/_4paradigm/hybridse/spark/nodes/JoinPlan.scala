...
import com._4paradigm.hybridse.spark.utils.{HybridseUtil, SparkColumnUtil, SparkRowUtil, SparkUtil}
...
object JoinPlan {
  ...
  def gen(ctx: PlanContext, node: PhysicalJoinNode, left: SparkInstance, right: SparkInstance): SparkInstance = {
    ...
    if (filter.condition() != null) {
      val regName = "NATIVE_SPARK_JOIN_CONDITION_" + filter.fn_info().fn_name()
