...
import com._4paradigm.hybridse.spark.utils.{HybridseUtil, HDFSUtil}
import org.apache.spark.sql.SparkSession
import org.slf4j.LoggerFactory



object NativeSparkMain {

  private val logger = LoggerFactory.getLogger(this.getClass)
  private val sparkMaster = "cluster"
  private val appName: String = "NativeSparkApp"
  ...
  def run(config: SparkConfig): Unit = {
    ...
    val nativeSparkConfig = NativeSparkConfig.fromSparkSession(sparkSession)

    val sess = new NativeSession(sparkSession)
    sess.version()
    ...
    for ((name, path) <- inputTables.asScala) {
      ...
      if (nativeSparkConfig.tinyData > 0) {
        sess.read(path).tiny(nativeSparkConfig.tinyData).createOrReplaceTempView(name)
      } else {
    ...
    val res = sess.nativeSparkSql(sqlScript)
    logger.info(s"output schema:${res.sparkDf.schema.toDDL}")

    res.sparkDf.show(100)
    logger.info("NativeSpark compute is done")
