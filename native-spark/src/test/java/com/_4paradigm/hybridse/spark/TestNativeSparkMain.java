...
public class TestNativeSparkMain {
    private static final Logger logger = LoggerFactory.getLogger(TestNativeSparkMain.class);
    ...
    public Object[][] getConfig() {
        ...
        return new Object[][] {
                new Object[] {
                        ...
                        "spark.master=local spark.nativespark.skew.watershed=1 spark.nativespark.test.print=false spark.nativespark.test.print.sampleInterval=1 spark.nativespark.test.print.printContent=false spark.nativespark.mode=skew spark.nativespark.skew.level=2 spark.nativespark.group.partitions=20 spark.sql.shuffle.partitions=8",
                        "spark.nativespark.mode=normal spark.master=local"
                },
                new Object[] {
                        "风电场景 时间窗口",
                        ...
                        "output/fengdian/skew",
                        "output/fengdian/no-skew",
                        "spark.master=local spark.nativespark.skew.watershed=1 spark.nativespark.test.print=false spark.nativespark.mode=skew spark.nativespark.skew.level=2 spark.nativespark.group.partitions=20 spark.sql.shuffle.partitions=8",
                        "spark.nativespark.mode=normal spark.master=local"
                },
                new Object[] {
                        "风电场景 时间和条数窗口混合",
    ...
    public void testSkewModeResult(String desc, String scriptPath, String jsonPath, String[] checkField, String output1, String output2, String config1, String config2) throws IOException {
        logger.info(desc);
        jsonPath = TestNativeSparkMain.class.getClassLoader().getResource(jsonPath).getPath();
        scriptPath = TestNativeSparkMain.class.getClassLoader().getResource(scriptPath).getPath();
        String root = TestNativeSparkMain.class.getClassLoader().getResource(".").getPath();
        FileUtils.forceMkdir(new File(root + "/" + output1));
        FileUtils.forceMkdir(new File(root + "/" + output2));
        output1 = TestNativeSparkMain.class.getClassLoader().getResource(output1).getPath();
        output2 = TestNativeSparkMain.class.getClassLoader().getResource(output2).getPath();
        ...
        NativeSparkMain.run(sc1);
        NativeSparkMain.run(sc2);
