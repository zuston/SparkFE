...
import com._4paradigm.hybridse.spark.{NativeSparkConfig, SparkPlanner}
import com._4paradigm.hybridse.spark.{NativeSparkConfig, SparkPlanner}
...
class NativeSession {
  ...
  private var config: NativeSparkConfig = _
  ...
  def this(sparkSession: SparkSession) = {
    ...
    this.config = NativeSparkConfig.fromSparkSession(sparkSession)
  ...
  def getSparkSession: SparkSession = {
    this.synchronized {
      if (this.sparkSession == null) {
        ...
        this.sparkSession = builder.appName("NativeSparkApp")
          .master(sparkMaster)
          .getOrCreate()
      }
   ...
   * @return
   */
  def read(filePath: String, format: String = "parquet"): NativeDataframe = {
    ...
    new NativeDataframe(this, sparkDf)
  }



  /**
   * Read the Spark dataframe to native dataframe.
   *
   * @param sparkDf
   * @return
   */
  def readSparkDataframe(sparkDf: DataFrame): NativeDataframe = {
    NativeDataframe(this, sparkDf)
  }
   ...
   * @return
   */
  def nativeSparkSql(sqlText: String): NativeDataframe = {
   ...
   * @return
   */
  def sql(sqlText: String): NativeDataframe = {
    nativeSparkSql(sqlText)
  }
   ...
   * @return
   */
  def sparksql(sqlText: String): NativeDataframe = {
    ...
    NativeDataframe(this, outputDataset)
  }
   ...
   */
  def version(): Unit = {
    val stream = this.getClass.getClassLoader.getResourceAsStream("native_spark_git.properties")
    if (stream == null) {
