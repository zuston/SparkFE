...
import com._4paradigm.hybridse.spark.utils.{HybridseUtil, SparkColumnUtil}
...
object FilterPlan {
  ...
  def gen(ctx: PlanContext, node: PhysicalFilterNode, input: SparkInstance): SparkInstance = {
    ...
    if (filter.condition() != null) {
      val regName = "NATIVE_SPARK_FILTER_CONDITION_" + node.filter().condition().fn_info().fn_name()
