...
import com._4paradigm.hybridse.spark.NativeSparkConfig
...
object ConfigReflections {
  ...
  private def createSetters(): Map[String, (NativeSparkConfig, Any) => Unit] = {
    val universe = scala.reflect.runtime.universe
    val mirror = universe.runtimeMirror(NativeSparkConfig.getClass.getClassLoader)
    val configType = universe.typeOf[NativeSparkConfig]
    val setters = mutable.HashMap[String, (NativeSparkConfig, Any) => Unit]()
    ...
    fields.foreach(symbol => {
      ...
      if (configOption != null) {
        ...
        if (name != null) {
          setters += name -> { (config: NativeSparkConfig, value: Any) =>
            try {
  ...
  def createConfig[T](source: T)(func: (T, String) => Any): NativeSparkConfig = {
    val config = new NativeSparkConfig
    applySetters(source, config)(func)
    config
  }

  def applySetters[T](source: T, config: NativeSparkConfig)(func: (T, String) => Any): Unit = {
